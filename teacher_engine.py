...
async def speak_text(user_id, message, text):
    ...
    # Matnni [en]...[/en] va o'zbek qismlarga bo'lamiz
    import re
    parts = re.split(r'\[en\](.*?)\[/en\]', str(text), flags=re.DOTALL)
    # parts: [uz_text, en_text, uz_text, en_text, ...]

    # Har qismni mos ovozda audio fayllarga yig'amiz
    segments = []
    for i, part in enumerate(parts):
        clean = tts_clean(part) if i % 2 == 0 else part.strip()
        # emoji va maxsus belgilarni tozalash
        clean = re.sub(r'[\U0001F000-\U0001FFFF\U00002600-\U000027BF]+', ' ', clean)
        clean = re.sub(r'\s+', ' ', clean).strip()
        if not clean or not any(c.isalnum() for c in clean):
            continue
        voice = en_voice if i % 2 == 1 else uz_voice
        segments.append((clean, voice))

    if not segments:
        return

    # Har segmentni alohida mp3 qilib birlashtiramiz
    from pydub import AudioSegment as AS
    import io
    combined = AS.silent(duration=0)

    for seg_text, voice in segments:
        tmp = f"tts_tmp_{user_id}_{id(seg_text)}.mp3"
        try:
            comm = edge_tts.Communicate(text=seg_text, voice=voice)
            await comm.save(tmp)
            if os.path.exists(tmp) and os.path.getsize(tmp) > 0:
                audio = AS.from_mp3(tmp)
                combined += audio
        except: pass
        finally:
            try: os.remove(tmp)
